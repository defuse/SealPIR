...
int main(int argc, char *argv[]) {
    uint64_t database_sizes[3] = {1000000, 2000000, 5000000};
    for (int db_size_i = 0; db_size_i < 3; db_size_i++) {
        uint64_t number_of_items = database_sizes[db_size_i];

        cout << "---------------------------------------------------------------" << endl;
        cout << "Benchmarking Database Size: " << number_of_items << endl;
        cout << "---------------------------------------------------------------" << endl;

        uint64_t size_per_item = 580; // in bytes
        uint32_t N = 2048;
        ...
        auto time_galois_s = high_resolution_clock::now();
        GaloisKeys galois_keys = client.generate_galois_keys();
        auto time_galois_e = high_resolution_clock::now();
        auto time_galois_us = duration_cast<microseconds>(time_galois_e - time_galois_s).count();

        std::ostringstream gks;
        galois_keys.save(gks);
        std::string gk = gks.str();
        cout << "Galois key size (bytes):" << gk.length() << endl;
        ...
        cout << "Query serialized length: " << s.length() << endl;
        //cout << s << endl;
        ...
        std::string s_reply = serialize_ciphertexts(reply);
        cout << "Reply serialized length: " << s_reply.length() << endl;
        ...
        // Output results cout << "Main: PIR result correct!" << endl;
        cout << "Main: PIRServer pre-processing time: " << time_pre_us / 1000 << " ms" << endl;
        cout << "Main: PIRClient galois key generation time: " << time_galois_us / 1000 << " ms" << endl;
